...
if os.path.exists(sqlite_path):
    ...
    print(
        f"--- [ERROR] No DB found at {sqlite_path}. Check your ingestion script path! ---")
...
class ContractDetails(BaseModel):
    document_type: str = Field(
        description="The type of document (e.g. Employment Contract, NDA)", default="Employment Contract")
    ...
    location: str = Field(
        description="Country (Malaysia, Singapore, Hong Kong)")
...
def clean_pdf_text(text):
    text = re.sub(r"This sample document is provided.*",
...
def get_template_text(location, doc_type):
    ...
    if "malaysia" in loc_key:
        target_folder = os.path.join(base_path, "malaysia")
    elif "singapore" in loc_key:
        target_folder = os.path.join(base_path, "singapore")
    elif "hong" in loc_key:
        target_folder = os.path.join(base_path, "hong-kong")
    else:
        target_folder = os.path.join(base_path, loc_key)
    ...
    if os.path.exists(target_folder):
        print(
            f"--- [SYSTEM] Searching in {target_folder} for keywords: {keywords} ---")
        ...
        if target_file:
            ...
            if target_file.endswith(".md"):
                ...
                pages = loader.load()
                found_text = "\n".join(
                    [clean_pdf_text(p.page_content) for p in pages])
        else:
            print(
                f"--- [WARNING] No file matched keywords {keywords} in {target_folder} ---")
...
def extract_data_node(state):
    ...
    return {"contract_data": state['contract_data']}
...
def contract_drafting_node(state):
    ...
    print(f"DATA RECEIVED FOR DRAFTING:\n{data}\n")

    prompt = ChatPromptTemplate.from_messages([
        ("system", """You are an expert HR Lawyer. Your task is to rewrite the provided Template into a professional document.
        
        YOU MUST FOLLOW THESE RULES EXACTLY BELOW. NO EXCEPTIONS.
        
        CRITICAL RULES:
        1. **Override Everything**: The provided JSON DATA is the absolute truth. Always use the values from Data to replace any corresponding placeholders or sections in the Template.
        2. **WORK COUNTRY**: The WORK COUNTRY is MALAYSIA. The "location" field in DATA specifies the LEGAL REFERENCE COUNTRY. You MUST use the right template and clauses based on the "location" field in DATA.
        3. **No Placeholders**: You are strictly forbidden from leaving text in brackets like [Current Date] or [Address]. If a piece of information is missing from the Data, use common sense (e.g., for Date, use today's date) or remove the bracketed section entirely.
        4. **Formatting**: Output clean MARKDOWN. 
        5. **Specific Mapping**: 
           - Map 'start_date' from Data to 'Employee Contract Agreement Date' and 'Commencement Date' in the document.
           - Map 'address' from Data to 'Employee Residential Address'.
        6. **Legal Accuracy**: Maintain the legal tone of the template, but ensure all names and numbers are updated."""),
        ("human",
         "DATA FROM FRONTEND:\n{data_input}\n\nRAW TEMPLATE TEXT:\n{template_input}")
    ])
...
def assistant_node(state):
    ...
    - Only say "I'm sorry, I don't have that specific policy information in my current records." if you cannot answer the user's query from the provided context. Do not include this sentence if you already provided a valid answer.
